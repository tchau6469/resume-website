            ...
            {items.map((item) => (
              <li key={item} className="my-4">
                <Link className="group flex items-center" href={`#${item}`}>
                  <div className="mr-4 h-[1px] w-8 bg-slate-500 transition-all duration-300 group-hover:w-16 group-hover:bg-slate-200 group-focus:w-16 group-focus:bg-slate-200"></div>
                  <div className="transition-color text-xs font-bold uppercase tracking-wide text-slate-500 duration-300 group-hover:text-slate-200 group-focus:text-slate-200">
                    {item}
                  </div>
                </Link>
              </li>
